...
/* ===== CHART FALLBACK + WITHDRAWAL LOGIC FIX ===== */
function cwMoney(n) {
...
function cwCoin() {
  return document.getElementById("coinSelect")?.value ||
         ...
         document.getElementById("managedCoin")?.value ||
         "BTCUSDT";
}

function cwPrice(coin = cwCoin()) {
  try {
    if (typeof priceOf === "function") return Number(priceOf(coin) || 0);
  } catch(e) {}
  return Number((window.state?.prices?.[coin]?.price) || 100);
}
...
function renderChartFallback() {
  ...
  const coin = cwCoin();
  const price = cwPrice(coin);
  const change = Number(window.state?.prices?.[coin]?.change || 0);

  hosts.forEach(host => {
    if (host.dataset.chartFixed === "1") return;

    // If TradingView iframe already exists, keep it but add fallback behind/below.
    let fallback = host.querySelector(".local-chart-fallback");
    if (!fallback) {
      fallback = document.createElement("div");
      fallback.className = "local-chart-fallback";
      fallback.innerHTML = `
        <div class="fallback-chart-head">
          <div>
            <b class="fallback-symbol">${coin.replace("USDT", "/USDT")}</b>
            <span>Live fallback chart</span>
          </div>
          <strong class="fallback-price">${price}</strong>
        </div>
        <div class="fallback-line-wrap">
          <svg class="fallback-svg" viewBox="0 0 320 130" preserveAspectRatio="none">
            <defs>
              <linearGradient id="fallbackGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stop-color="rgba(0,229,155,.45)"></stop>
                <stop offset="100%" stop-color="rgba(0,229,155,0)"></stop>
              </linearGradient>
            </defs>
            <path class="fallback-area" d=""></path>
            <path class="fallback-line" d=""></path>
          </svg>
        </div>
        <div class="fallback-chart-foot">
          <span>Chart fallback active</span>
          <em class="fallback-change">${change.toFixed(2)}%</em>
        </div>
      `;
    ...
    let seed = price || 100;
    for (let i = 0; i < 28; i++) {
      seed += (Math.sin(i * 0.9 + Date.now() / 20000) * price * 0.0006) + ((Math.random() - 0.5) * price * 0.0008);
      pts.push(seed);
    }
    ...
    const area = line + " L 320 130 L 0 130 Z";

    fallback.querySelector(".fallback-symbol").textContent = coin.replace("USDT", "/USDT");
    fallback.querySelector(".fallback-price").textContent = "$" + Number(price || 0).toLocaleString("en-US", { maximumFractionDigits: 2 });
    fallback.querySelector(".fallback-change").textContent = change.toFixed(2) + "%";
    fallback.querySelector(".fallback-line").setAttribute("d", line);
    fallback.querySelector(".fallback-area").setAttribute("d", area);

    // If host has no iframe/canvas, show fallback normally.
    const hasRealChart = !!host.querySelector("iframe, canvas");
    fallback.classList.toggle("is-backup", hasRealChart);
  });
}

function forceReloadTradingViewChart() {
  const coin = cwCoin();
  const symbol = "BINANCE:" + coin;
  const frames = document.querySelectorAll("iframe");
  frames.forEach(frame => {
    const src = frame.getAttribute("src") || "";
    if (src.includes("tradingview") || src.includes("tv")) {
      try {
...
function walletUserIdFinal() {
  try {
    if (typeof userKey === "function") return userKey();
  } catch(e) {}
  return String(window.state?.user?.id || window.state?.user?.email || "local");
}

function walletLedgerForUserFinal(uid = walletUserIdFinal()) {
  return (window.state?.walletLedger || []).filter(l => String(l.userId || l.user_id || "") === String(uid));
}

function approvedDepositFinal(uid = walletUserIdFinal()) {
  const reqTotal = (window.state?.depositRequests || [])
    .filter(d => String(d.userId || d.user_id || "") === String(uid) && String(d.status || "").toUpperCase() === "APPROVED")
    .reduce((a, d) => a + Number(d.amount || 0), 0);

  const ledgerTotal = walletLedgerForUserFinal(uid)
    .filter(l => String(l.type || "").toUpperCase() === "DEPOSIT")
    .reduce((a, l) => a + Number(l.amount || 0), 0);

  return Math.max(reqTotal, ledgerTotal);
}

function approvedWithdrawalFinal(uid = walletUserIdFinal()) {
  const reqTotal = (window.state?.withdrawalRequests || [])
    .filter(w => String(w.userId || w.user_id || "") === String(uid) && String(w.status || "").toUpperCase() === "APPROVED")
    .reduce((a, w) => a + Number(w.amount || 0), 0);

  const ledgerTotal = walletLedgerForUserFinal(uid)
    .filter(l => String(l.type || "").toUpperCase() === "WITHDRAWAL")
    .reduce((a, l) => a + Math.abs(Number(l.amount || 0)), 0);

  return Math.max(reqTotal, ledgerTotal);
}

function pendingWithdrawalFinal(uid = walletUserIdFinal()) {
  return (window.state?.withdrawalRequests || [])
    .filter(w => String(w.userId || w.user_id || "") === String(uid) && String(w.status || "").toUpperCase() === "PENDING")
    .reduce((a, w) => a + Number(w.amount || 0), 0);
}

function profitBonusFinal(uid = walletUserIdFinal()) {
  return walletLedgerForUserFinal(uid)
    .filter(l => ["TRADE_PNL", "MANAGED_TRADE_PNL", "MASS_TRADE_PNL", "REFERRAL_BONUS"].includes(String(l.type || "").toUpperCase()))
    .reduce((a, l) => a + Number(l.amount || 0), 0);
}

function completedTradeVolumeFinal(uid = walletUserIdFinal()) {
  const real = window.state?.accounts?.REAL || { closedTrades: [] };

  const manual = (real.closedTrades || [])
    .filter(t => !t.userId || String(t.userId) === String(uid))
    .reduce((a, t) => a + Number(t.amount || 0), 0);

  const managed = (window.state?.managedTrades || [])
    .filter(t => String(t.userId || t.user_id || "") === String(uid) && String(t.status || "").toUpperCase() === "CLOSED")
    .reduce((a, t) => a + Number(t.amount || 0), 0);

  return manual + managed;
}

function withdrawableFinal(uid = walletUserIdFinal()) {
  const deposits = approvedDepositFinal(uid);
  const traded = completedTradeVolumeFinal(uid);

  // Deposit unlock only up to completed trade volume.
  const unlockedDeposit = Math.min(deposits, traded);

  // Positive profit/referral bonus is withdrawable. Loss reduces wallet but does not create negative withdrawal.
  const profitBonus = Math.max(0, profitBonusFinal(uid));

  return Math.max(0, unlockedDeposit + profitBonus - approvedWithdrawalFinal(uid) - pendingWithdrawalFinal(uid));
}

function totalWalletFinal(uid = walletUserIdFinal()) {
  return Math.max(0, approvedDepositFinal(uid) + profitBonusFinal(uid) - approvedWithdrawalFinal(uid));
}

// Override old withdrawal helpers if they exist.
try { approvedDeposits = approvedDepositFinal; } catch(e) {}
try { approvedWithdrawals = approvedWithdrawalFinal; } catch(e) {}
try { ledgerPnL = profitBonusFinal; } catch(e) {}
try { tradeVolume = completedTradeVolumeFinal; } catch(e) {}
try { withdrawable = withdrawableFinal; } catch(e) {}
try { realWallet = totalWalletFinal; } catch(e) {}

window.withdrawableFinal = withdrawableFinal;
window.totalWalletFinal = totalWalletFinal;

function renderWithdrawalBreakdownFinal() {
  const uid = walletUserIdFinal();
  const deposits = approvedDepositFinal(uid);
  const traded = completedTradeVolumeFinal(uid);
  const unlocked = Math.min(deposits, traded);
  const profit = profitBonusFinal(uid);
  const pending = pendingWithdrawalFinal(uid);
  const available = withdrawableFinal(uid);
  const total = totalWalletFinal(uid);

  const map = {
    walletBalance: total,
    walletPageBalance: total,
    mockRealBalance: total,
    approvedDepositText: deposits,
    tradeVolumeText: traded,
    profitEligibleText: profit,
    pendingWithdrawalText: pending,
    withdrawableAmountText: available,
    availableWithdrawText: available,
    withdrawalAvailableText: available,
    unlockedDepositText: unlocked
  };

  Object.entries(map).forEach(([id, val]) => {
    const el = document.getElementById(id);
    if (el) el.textContent = cwMoney(val);
  });

  // Create a small helper card if wallet page has no breakdown.
  const walletPage = document.getElementById("wallet");
  if (walletPage && !document.getElementById("withdrawBreakdownAuto")) {
    const card = document.createElement("div");
    card.id = "withdrawBreakdownAuto";
    card.className = "card withdraw-breakdown-auto";
    card.innerHTML = `
      <p class="label">Withdrawal Calculation</p>
      <div class="withdraw-break-grid">
        <div><span>Approved Deposit</span><b id="wbDeposit">₹0</b></div>
        <div><span>Trade Volume Done</span><b id="wbVolume">₹0</b></div>
        <div><span>Unlocked Deposit</span><b id="wbUnlocked">₹0</b></div>
        <div><span>Profit / Bonus</span><b id="wbProfit">₹0</b></div>
        <div><span>Pending Withdrawal</span><b id="wbPending">₹0</b></div>
        <div><span>Available Withdrawal</span><b id="wbAvailable">₹0</b></div>
      </div>
      <p class="muted small">Deposit amount उतना ही withdraw होगा जितने amount से completed trade हो चुकी है. Profit/bonus अलग से add होगा.</p>
    `;
    walletPage.appendChild(card);
  }
  ...
  Object.entries(bmap).forEach(([id, val]) => {
    const el = document.getElementById(id);
    if (el) el.textContent = cwMoney(val);
  });
}

function validateWithdrawalBeforeSubmitFinal() {
  const amount = Number(document.getElementById("withdrawAmount")?.value || 0);
  if (!amount || amount < 1000) {
    alert("Minimum withdrawal ₹1000 hai.");
    return false;
  }
  const available = withdrawableFinal();
  if (amount > available) {
    alert("Available withdrawal se jyada amount nahi nikal sakte. Available: " + cwMoney(available));
    return false;
  }
  return true;
}

// Capture withdrawal submit before old logic if amount is invalid.
document.addEventListener("click", function(e) {
  const btn = e.target.closest("#submitWithdrawRequest, #withdrawSubmitBtn, #submitWithdrawalBtn, [data-action='withdraw-submit']");
  if (!btn) return;
  if (!validateWithdrawalBeforeSubmitFinal()) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    return false;
...
document.addEventListener("change", function(e) {
  if (e.target && ["coinSelect", "tradePairSelect"].includes(e.target.id)) {
    setTimeout(forceReloadTradingViewChart, 150);
  }
}, true);
...
setInterval(function(){
  try {
    renderChartFallback();
    renderWithdrawalBreakdownFinal();
  } catch(e) {}
}, 1500);
