  ...
  function walletHtml(){
    ...
    return `
      ...
      <section class="pro-wallet-cards">
        ...
        ${metricCard("Pending Deposit", w.pendingDeposit, "Waiting for approval", "⏳")}
...
/* ===== WALLET BALANCE TRADE LOGIC FINAL ===== */
(function(){
  const DEP_TABLE = "deposit_requests";
  const WIT_TABLE = "withdrawal_requests";
  ...
  function user(){ return state?.user || {}; }
  function currentUserId(){ const u=user(); return String(u.id || u.email || "local"); }
  function norm(v){ return String(v || "").trim().toLowerCase(); }
  ...
  function toastMsg(msg){ try { toast?.(msg); } catch(e){ console.log(msg); } }

  function getUserById(id){
    id = String(id || "");
    return (state.users || []).find(u => String(u.id) === id || String(u.email) === id);
  }
  function ensureAccount(userId){
    state.accounts ||= {};
    const id = String(userId || currentUserId());
    state.accounts[id] ||= {};
    if (state.accounts[id].balance == null) {
      const u = getUserById(id);
      state.accounts[id].balance = Number(u?.balance || u?.wallet || 0);
    }
    return state.accounts[id];
  }
  function getBalance(userId){
    const acc = ensureAccount(userId);
    return Number(acc.balance || acc.realBalance || 0);
  }
  function setBalance(userId, amount){
    const acc = ensureAccount(userId);
    acc.balance = Math.max(0, Number(amount || 0));
    acc.realBalance = acc.balance;
    const u = getUserById(userId);
    if (u) u.balance = acc.balance;
    if (String(user()?.id || user()?.email) === String(userId)) {
      state.user.balance = acc.balance;
    }
    save();
    return acc.balance;
  }
  function addBalance(userId, amount){
    return setBalance(userId, getBalance(userId) + Number(amount || 0));
  }
  function deductBalance(userId, amount){
    amount = Number(amount || 0);
    const bal = getBalance(userId);
    if (amount > bal) return false;
    setBalance(userId, bal - amount);
    return true;
  }
  function sameUser(row){
    const uid = currentUserId();
    const em = norm(user().email);
    return String(row.userId || row.user_id || "") === uid || norm(row.userEmail || row.user_email || "") === em;
  }
  function depRows(){
    state.depositRequests ||= [];
    return state.depositRequests.map(d => ({
      ...
      utr:String(d.utr || d.transactionId || d.transaction_id || d.txn || d.txn_id || ""),
      status:String(d.status || "PENDING").toUpperCase(),
      balanceApplied: !!(d.balanceApplied || d.balance_applied),
      holdApplied: !!(d.holdApplied || d.hold_applied),
      createdAt:String(d.createdAt || d.created_at_text || d.created_at || "")
    }));
  }
  function witRows(){
    ...
    return state.withdrawalRequests.map(w => ({
      ...
      holderName:String(w.holderName || w.holder_name || ""),
      status:String(w.status || "PENDING").toUpperCase(),
      holdApplied: !!(w.holdApplied || w.hold_applied),
      createdAt:String(w.createdAt || w.created_at_text || w.created_at || "")
    }));
  }

  function walletNumbers(userId){
    const uid = String(userId || currentUserId());
    const u = getUserById(uid) || user();
    const em = norm(u.email || user().email);
    const same = r => String(r.userId || r.user_id || "") === uid || norm(r.userEmail || r.user_email || "") === em;
    const available = getBalance(uid);
    const pendingDeposit = depRows().filter(d => same(d) && d.status === "PENDING").reduce((s,d)=>s+d.amount,0);
    const pendingWithdrawal = witRows().filter(w => same(w) && w.status === "PENDING").reduce((s,w)=>s+w.amount,0);
    const approvedDeposit = depRows().filter(d => same(d) && d.status === "APPROVED").reduce((s,d)=>s+d.amount,0);
    return {
      available,
  ...
  function patchProfessionalWalletData(){
    // Override walletData used by the professional wallet module if accessible through closures is impossible;
    // instead update DOM after render and keep calculations aligned.
    const nums = walletNumbers();
    const root = document.getElementById("professionalWalletRoot");
    if (!root) return;

    const heroAmount = root.querySelector(".pro-wallet-hero strong");
    if (heroAmount) heroAmount.textContent = money(nums.available);

    const cards = Array.from(root.querySelectorAll(".pro-wallet-metric"));
    cards.forEach(card => {
      const title = (card.querySelector("p")?.textContent || "").toLowerCase();
      const val = card.querySelector("b");
      const sub = card.querySelector("small");
      if (!val) return;
      if (title.includes("available")) val.textContent = money(nums.available);
      if (title.includes("withdrawable")) val.textContent = money(nums.withdrawable);
      if (title.includes("approved deposit")) {
        card.querySelector("p").textContent = "Pending Deposit";
        val.textContent = money(nums.pendingDeposit);
        if (sub) sub.textContent = "Waiting for approval";
      }
      if (title.includes("pending deposit")) val.textContent = money(nums.pendingDeposit);
      if (title.includes("pending withdrawal")) val.textContent = money(nums.pendingWithdrawal);
    });
  }
  ...
  async function dbUpdate(table, id, patch){
    const db = client();
    if (!db) return;
    try { await db.from(table).update(patch).eq("id", id); } catch(e){}
  }

  // Deposit approve: add amount once. Reject: no add.
  async function approveDepositFixed(id){
    const r = (state.depositRequests || []).find(x => String(x.id) === String(id));
    if (!r || String(r.status || "").toUpperCase() !== "PENDING") return;
    r.status = "APPROVED";
    if (!r.balanceApplied && !r.balance_applied) {
      addBalance(r.userId || r.user_id, Number(r.amount || 0));
      r.balanceApplied = true;
      r.balance_applied = true;
    }
    await dbUpdate(DEP_TABLE, id, { status:"APPROVED", balance_applied:true, reviewed_at:new Date().toISOString() });
    save();
    refreshWalletAndAdmin();
  }
  async function rejectDepositFixed(id){
    const r = (state.depositRequests || []).find(x => String(x.id) === String(id));
    if (!r || String(r.status || "").toUpperCase() !== "PENDING") return;
    r.status = "REJECTED";
    await dbUpdate(DEP_TABLE, id, { status:"REJECTED", reviewed_at:new Date().toISOString() });
    save();
    refreshWalletAndAdmin();
  }

  // Withdrawal submit hold: deduct instantly. Approve: no extra deduct. Reject: refund once.
  function applyWithdrawalHold(row){
    if (!row || row.holdApplied || row.hold_applied) return true;
    const userId = row.userId || row.user_id || currentUserId();
    const amount = Number(row.amount || 0);
    if (!deductBalance(userId, amount)) return false;
    row.holdApplied = true;
    row.hold_applied = true;
    return true;
  }
  async function approveWithdrawalFixed(id){
    const r = (state.withdrawalRequests || []).find(x => String(x.id) === String(id));
    if (!r || String(r.status || "").toUpperCase() !== "PENDING") return;
    // If old request did not hold yet, hold now so approved payout is still deducted.
    if (!r.holdApplied && !r.hold_applied) {
      const ok = applyWithdrawalHold(r);
      if (!ok) return alert("User balance is not enough for this withdrawal.");
    }
    r.status = "APPROVED";
    await dbUpdate(WIT_TABLE, id, { status:"APPROVED", hold_applied:true, reviewed_at:new Date().toISOString() });
    save();
    refreshWalletAndAdmin();
  }
  async function rejectWithdrawalFixed(id){
    const r = (state.withdrawalRequests || []).find(x => String(x.id) === String(id));
    if (!r || String(r.status || "").toUpperCase() !== "PENDING") return;
    if (r.holdApplied || r.hold_applied) {
      addBalance(r.userId || r.user_id, Number(r.amount || 0));
      r.holdApplied = false;
      r.hold_applied = false;
    }
    r.status = "REJECTED";
    await dbUpdate(WIT_TABLE, id, { status:"REJECTED", hold_applied:false, reviewed_at:new Date().toISOString() });
    save();
    refreshWalletAndAdmin();
  }

  function patchWithdrawalSubmit(){
    // Intercept the final submit button after professional wallet renders.
    document.addEventListener("click", function(e){
      const btn = e.target.closest("#proWitSubmit");
      if (!btn) return;
      // Let original handler create row first, then hold the latest pending row after a tick.
      setTimeout(async () => {
        const latest = (state.withdrawalRequests || []).find(w => 
          String(w.userId || w.user_id || "") === currentUserId() &&
          String(w.status || "").toUpperCase() === "PENDING" &&
          !(w.holdApplied || w.hold_applied)
        );
        if (latest) {
          const ok = applyWithdrawalHold(latest);
          if (!ok) {
            latest.status = "REJECTED";
            toastMsg("Withdrawal rejected: insufficient balance.");
          } else {
            await dbUpdate(WIT_TABLE, latest.id, { hold_applied:true });
          }
          save();
          refreshWalletAndAdmin();
        }
  ...
  /* Trade balance logic:
     - Open trade: deduct trade amount once.
     - Close profit: return amount + profit.
     - Close loss: return amount - loss.
  */
  function tradeUserId(t){
    return String(t.userId || t.user_id || t.userEmail || t.user_email || currentUserId());
  }
  function tradeAmount(t){
    return Number(t.amount || t.tradeAmount || t.margin || t.stake || t.investment || 0);
  }
  function tradePnl(t){
    return Number(t.pnl || t.profitLoss || t.profit_loss || t.resultAmount || t.result_amount || 0);
  }
  function isTradeClosed(t){
    const s = String(t.status || t.state || "").toUpperCase();
    return ["CLOSED","COMPLETE","COMPLETED","SETTLED","WIN","LOSS"].includes(s);
  }
  function isTradeOpen(t){
    const s = String(t.status || t.state || "OPEN").toUpperCase();
    return ["OPEN","ACTIVE","RUNNING","LIVE"].includes(s);
  }
  function applyTradeOpenHold(t){
    if (!t || t.walletHoldApplied || t.wallet_hold_applied) return true;
    const amt = tradeAmount(t);
    if (amt <= 0) return true;
    const uid = tradeUserId(t);
    if (!deductBalance(uid, amt)) return false;
    t.walletHoldApplied = true;
    t.wallet_hold_applied = true;
    save();
    return true;
  }
  function settleClosedTrade(t){
    if (!t || t.walletSettled || t.wallet_settled) return;
    const amt = tradeAmount(t);
    if (amt <= 0) return;
    const uid = tradeUserId(t);
    let pnl = tradePnl(t);

    // Fallback: if status says LOSS and pnl is positive, make it negative.
    const st = String(t.status || "").toUpperCase();
    if (st === "LOSS" && pnl > 0) pnl = -pnl;
    if (st === "WIN" && pnl < 0) pnl = Math.abs(pnl);

    // If trade was not held before, deduct first to normalize old trades.
    if (!(t.walletHoldApplied || t.wallet_hold_applied)) {
      const ok = applyTradeOpenHold(t);
      if (!ok) return;
    }

    const returnAmount = Math.max(0, amt + pnl);
    addBalance(uid, returnAmount);
    t.walletSettled = true;
    t.wallet_settled = true;
    save();
  }
  function scanTrades(){
    const buckets = ["managedTrades","manualTrades","trades","openTrades","tradeHistory"];
    buckets.forEach(key => {
      const arr = state[key];
      if (!Array.isArray(arr)) return;
      arr.forEach(t => {
        if (isTradeClosed(t)) settleClosedTrade(t);
        else if (isTradeOpen(t)) applyTradeOpenHold(t);
      });
    });
    patchProfessionalWalletData();
  }

  function refreshWalletAndAdmin(){
    try { window.renderProfessionalWallet?.(); } catch(e){}
    try { window.renderAdminDepositsClean?.(); } catch(e){}
    try { window.renderAdminWithdrawalsClean?.(); } catch(e){}
    setTimeout(patchProfessionalWalletData, 80);
    setTimeout(scanTrades, 120);
  }

  function patchGlobals(){
    window.walletNumbersFinal = walletNumbers;
    window.approveDepositRequest = approveDepositFixed;
    window.rejectDepositRequest = rejectDepositFixed;
    window.approveWithdrawalRequest = approveWithdrawalFixed;
    window.rejectWithdrawalRequest = rejectWithdrawalFixed;
    window.applyTradeWalletHold = applyTradeOpenHold;
    window.settleTradeWalletBalance = settleClosedTrade;
    window.scanTradeWalletBalances = scanTrades;

    const oldRender = window.renderProfessionalWallet;
    if (typeof oldRender === "function" && !window.__walletBalanceTradeRenderPatch) {
      window.__walletBalanceTradeRenderPatch = true;
      window.renderProfessionalWallet = function(){
        const res = oldRender.apply(this, arguments);
        setTimeout(patchProfessionalWalletData, 50);
        setTimeout(scanTrades, 100);
        return res;
      };
    }
  }
  ...
  document.addEventListener("click", function(e){
    const text = (e.target?.textContent || "").toLowerCase();
    const page = e.target.closest("[data-page]")?.dataset?.page || "";
    if (page === "wallet" || text.includes("wallet") || text.includes("trade") || text.includes("close") || text.includes("approve") || text.includes("reject")) {
      setTimeout(refreshWalletAndAdmin, 250);
      setTimeout(refreshWalletAndAdmin, 900);
    }
  }, true);

  function boot(){
    patchGlobals();
    patchWithdrawalSubmit();
    scanTrades();
    patchProfessionalWalletData();
  }
  ...
  window.addEventListener("load", () => {
    setTimeout(boot, 1200);
    setTimeout(refreshWalletAndAdmin, 2600);
  });
})();
